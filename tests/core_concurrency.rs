...
fn simultaneous_connections_allocate_unique_persistent_revisions() {
    ...
    let threads: Vec<_> = (0..8).map(|_| {
        ...
        std::thread::spawn(move || {
            ...
            barrier.wait();
            vault.propose("shared", "D", "C", "E", 0).unwrap().version
        })
    }).collect();
...
fn competing_publishers_have_exactly_one_winner() {
    ...
    let mut vault = Vault::open(&path).unwrap();
    for _ in 0..2 {
        vault.propose("shared", "D", "C", "E", 0).unwrap();
    }
    let barrier = Arc::new(Barrier::new(2));
    let threads: Vec<_> = (1..=2).map(|version| {
        let path = path.clone();
        let barrier = barrier.clone();
        std::thread::spawn(move || {
            let mut vault = Vault::open(&path).unwrap();
    ...
    let winners = threads.into_iter().filter_map(|t| t.join().unwrap().then_some(())).count();
    assert_eq!(winners, 1);
    assert_eq!(vault.drafts().unwrap().len(), 1);
    assert!(vault.get("shared", None).unwrap().published);
}
...
fn a_writer_waits_for_an_existing_write_transaction() {
    ...
    let thread = std::thread::spawn(move || {
        send.send(()).unwrap();
        vault.propose("shared", "D", "C", "E", 0)
    });
    receive.recv().unwrap();
    std::thread::sleep(std::time::Duration::from_millis(150));
    conn.execute_batch("COMMIT").unwrap();
    assert_eq!(thread.join().unwrap().unwrap().version, 1);
}
